...
export default function ChatBubble({
  ...
  const content = mentions.length ? renderWithMentions(body, mentions, activeName, peopleIndex, isUser) : body;
...
function findCategoryByName(peopleIndex, matched) {
  const target = matched.toLowerCase();
  for (const person of peopleIndex.values()) {
    const aliases = [person?.name, person?.fullName, ...(person?.aliases || [])]
      .filter(Boolean)
      .map(value => value.toLowerCase());
    if (aliases.includes(target)) return person.category || 'family';
  }
  return 'family';
}
