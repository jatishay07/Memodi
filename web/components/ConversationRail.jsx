      ...
      {[0, 0.12, 0.24, 0.36, 0.48].map((delay, i) => (
        <span
          ...
          style={{
            ...
            background: i % 2 === 0 ? '#DC4F7C' : '#C42B34',
...
export default function ConversationRail({
  ...
  const captionText  = isListening ? "I'm listening"
                     : isThinking  ? "I'm thinking…"
                     : isSpeaking  ? 'Memodi is speaking'
                     : 'Tap to speak with me';
  const captionColor = (isListening || isSpeaking) ? '#5C2238' : '#6B6B6B';

  const micBg      = isListening ? '#C42B34'
                   : 'linear-gradient(135deg, #FC8A2D 0%, #DC4F7C 100%)';
  const micLabel   = isListening ? 'Tap to stop'
                   : isThinking  ? 'Thinking…'
  ...
  return (
    <div style={{
      ...
      {/* Orb — 200px in conversation mode */}
      <div style={{
        transition: reducedMotion ? 'none' : 'width 0.7s cubic-bezier(.45,.05,.55,.95), height 0.7s cubic-bezier(.45,.05,.55,.95)',
      }}>
      ...
      <div style={{
        ...
        letterSpacing: '0.02em', color: captionColor,
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
        <button
          ...
          onMouseDown={e => { if (!isThinking) e.currentTarget.style.transform = 'scale(0.94)'; }}
          onMouseUp={e => { e.currentTarget.style.transform = 'scale(1)'; }}
          onMouseLeave={e => { e.currentTarget.style.transform = 'scale(1)'; }}
          style={{
            width: 80, height: 80, borderRadius: 999, border: 0,
            background: micBg,
            color: '#fff', cursor: isThinking ? 'not-allowed' : 'pointer',
            opacity: isThinking ? 0.7 : 1,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            animation: isListening && !reducedMotion ? 'micPulse 1.5s ease-out infinite' : 'none',
            boxShadow: isListening
              ? '0 0 0 0 rgba(220,79,124,0.5)'
              : '0 10px 32px rgba(220,79,124,0.32)',
        ...
        <span style={{
          fontFamily: 'var(--font-body)', fontSize: 14 * textScale,
          fontWeight: 500, color: '#6B6B6B',
        }}>
          {micLabel}
        </span>
      </div>
