...
DEFAULT_MODEL = ROOT / "voices" / f"{DEFAULT_VOICE}.onnx"
MODEL_PATH = Path(os.environ.get("PIPER_MODEL", DEFAULT_MODEL))

if not MODEL_PATH.exists():
    raise FileNotFoundError(f"Piper voice not found at {MODEL_PATH}")
...
def synthesize(req: SynthesizeRequest):
    ...
    return {"audioBase64": base64.b64encode(audio).decode("ascii"), "mimeType": "audio/wav"}
